...
ambClusterFind = function(rawObj, finalCBs = NULL, ambCutoffCoef = 1.5, batchCorrect = T, timesExcess = 2, res = 0.5, npcs = 30, ncores = 8){
	...
	rawObj$is_empty = ifelse(rownames(rawObj@meta.data) %in% rownames(finalCBs), 'Non_empty', 'Empty')
...
ambMarkFind = function(seuratObject, sorted = F, type, logfc = 0.25){
	...
	if(sorted == F){
        ...
        intronicCutoff <- c(quantile(ambientMeta$intronRat, 0.2),quantile(ambientMeta$intronRat, 0.8))
		...
		if(type == 'Abundance'){
			...
			highMat = GetAssayData(ambientObj, slot = "counts", assay = "RNA")[, highCB] %>% rowSums %>% prop.table * 100

			lowCB = rownames(ambientMeta[ambientMeta$ambient_intronic_type == 'Low_Intronic',])
			lowMat = GetAssayData(ambientObj, slot = "counts", assay = "RNA")[, lowCB] %>% rowSums %>% prop.table * 100
...
subCLEAN = function(object, group.by, key, ambientMarkers, batchCorrect = T, res = 0.5, npcs = 10){
	...
	# Subset cells
    seurat_clusters <- object[[group.by]]
    seurat_clusters <- unlist(seurat_clusters)
	...
	bcg = sum(rowMeans(subObj@assays$RNA@layers$counts) > 0.05)
